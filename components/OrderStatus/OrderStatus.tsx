...
const OrderStatus = ({ order }: OrderStatusPropsType) => {
  const orderSteps: { [key: string]: number } = {
    PENDING: 1,
    PROCESSING: 2,
    COMPLETE: 3,
    ON_HOLD: 4,
  ...
  return (
    <div id="order-status" className="space-y-12">
      ...
      {order?.status && (orderSteps[order?.status as string] as number) < 4 && (
        <div className="border-t w-full border-gray-200 sm:pb-12">
          ...
          <div className="pt-12 h-16" aria-hidden="true">
            ...
            <div className="mt-6 hidden grid-cols-4 text-sm font-medium text-gray-600 sm:grid">
              ...
              <div
                className={`${
                  orderSteps[order?.status] > 0 ? "text-accent" : ""
                } text-center`}
              >
              ...
              <div
                className={`${
                  orderSteps[order?.status] > 1 ? "text-accent" : ""
                } text-center`}
              >
                Processing
              </div>
              <div
                className={`${
                  orderSteps[order?.status] > 2 ? "text-accent" : ""
                } text-right`}
              >
                Shipped
              </div>
