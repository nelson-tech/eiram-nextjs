import { Collection, MediaItem } from "@api/codegen/graphql"
...
const Collection = ({ collection }: CollectionInputType) => {
  ...
  return (
    <div className="p-8 mx-auto max-w-7xl text-center text-gray-600">
      <h2 className="text-4xl font-bold tracking-wide pb-8">
        {collection?.title}
      </h2>
      ...
      {images && <CollectionGallery images={images as MediaItem[]} />}
      {collection?.gallery?.media?.video?.mediaItemUrl && (
