...
const useCheckout = (stripeData: STRIPE_PaymentIntentType) => {
  ...
  const createOrder = async ({ input, metadata }: CreateOrderPropsType) => {
    ...
    const checkoutData = await client.request(CheckoutDocument, {
      input: finalCheckoutInput,
    })
    ...
    if (checkoutData?.checkout?.result) {
      ...
      orderNumber &&
        router.push(
          `/thanks${orderNumber ? `?orderNumber=${orderNumber}` : ""}`,
          {
      ...
      switch (stripeResult?.paymentIntent.status) {
        case "processing":
          ...
          openAlert({
            ...
            secondary:
              "We'll create the order so it's ready as soon as the payment is complete.",
            timeout: 40000,
          })
