...
const InnerImageZoom = ({
	...
	const handleLoad = (e: { target: MutableRefObject<HTMLImageElement>["current"] }) => {
	...
	const handleDragMove = useCallback(
		(e: Event) => {
			...
			left = Math.max(
				Math.min(left, 0),
				((imgProps.current.scaledDimensions?.width ?? 0) - (imgProps.current.bounds?.width ?? 0)) *
					-1,
			)
			top = Math.max(
				Math.min(top, 0),
	...
	const handleFadeOut = (e: TransitionEvent<HTMLImageElement>, noTransition: boolean) => {
		if (noTransition || (e.propertyName === "opacity" && img.current?.contains(e.target as Node))) {
	...
	var initialDrag = function initialDrag(pageX: number, pageY: number) {
		...
		initialPageX =
			initialPageX +
			(isFullscreen ? (window.innerWidth - (imgProps.current.bounds?.width ?? 0)) / 2 : 0)
		initialPageY =
			initialPageY +
			(isFullscreen ? (window.innerHeight - (imgProps.current.bounds?.height ?? 0)) / 2 : 0)
	...
	const getRatios = (
		...
		return {
			x: ((dimensions.width ?? 0) - (bounds?.width ?? 0)) / (bounds?.width ?? 0),
			y: ((dimensions.height ?? 0) - (bounds?.height ?? 0)) / (bounds?.height ?? 0),
		}
	}
